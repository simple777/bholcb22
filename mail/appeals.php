...
// Настройки SMTP:
$smtp_host     = 'smtp.yandex.ru'; // SMPT-адрес сервера
$smtp_port     = 465; // TCP-порт
$smtp_secure   = 'ssl'; // SMTP TLS/SSL
$smtp_auth     = true; // SMPT-аутентификация
$smtp_username = 'B-Hol-CB@yandex.ru'; // Почтовый ящик, с которого будут отправляться письма
$smtp_password = 'BCB12345678'; // Пароль почтового ящика, с которого будут отправляться письма
...
try {
  ...
  if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
    ...
    if( $_POST['js'] === 'on' )  { header('sendmail: 1'); echo "success"; } //header('sendmail: 1'); 2021-07-08 me
